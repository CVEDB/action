...
function create_structure() {
    if [[ $project ]] 
    then
                CREATE_PROJECT="echo \"Y\"| cvedb create --space \"$space\" --project \"$project\""
                eval $CREATE_PROJECT
                PROJECT_CMD="--project \"$project\""
                SPACE_CMD="--space \"$space\""
    else
                CREATE_SPACE="echo \"Y\" | cvedb create --space \"$space\""
                eval $CREATE_SPACE
                PROJECT_CMD=""
                SPACE_CMD="--space \"$space\""
    fi
...
function execute_workflow() {
            ...
            max_machines
            # Create Command
            CVEDB_CMD="cvedb execute --ci $PROJECT_CMD $SPACE_CMD $WORKFLOW_CMD $WORKFLOW_NAME_CMD $CONFIG_CMD $WATCH_CMD $OUTPUT_DIR $OUTPUT_ALL $OUTPUT_NODES $SHOW_PARAMS_CMD $MAX_MACHINES_CMD"
            # Print Command
            echo "Command: $CVEDB_CMD"
            # Execute
            eval $CVEDB_CMD
}
